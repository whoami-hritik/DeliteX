...
async function deploy() {
    console.log("Generating deployer keypair for Fiat Ramp Router...");
    ...
    async function deployContract(wasmPath) {
        ...
        console.log(`\n🎉 Deployed Fiat Ramp Router Contract ID: ${contractId}`);
        return contractId;
    }
    
    try {
        let rampWasm = path.join(__dirname, "../target/wasm32v1-none/release/ramp_settlement.wasm");
        if (!fs.existsSync(rampWasm)) {
            rampWasm = path.join(__dirname, "../target/wasm32-unknown-unknown/release/ramp_settlement.wasm");
        }
        const rampId = await deployContract(rampWasm);

        console.log("\n--- FIAT RAMP ROUTER DEPLOYMENT SUCCESS ---");
        console.log(`NEXT_PUBLIC_SOROBAN_RAMP="${rampId}"`);
        ...
        envContent = envContent.replace(/^NEXT_PUBLIC_SOROBAN_RAMP=.*$/m, "");
        envContent += `\nNEXT_PUBLIC_SOROBAN_RAMP="${rampId}"\n`;
        
        fs.writeFileSync(envPath, envContent.trim() + "\n");
        console.log("Updated apps/web/.env.local with NEXT_PUBLIC_SOROBAN_RAMP");
