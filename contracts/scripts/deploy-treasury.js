...
async function deploy() {
    console.log("Generating deployer keypair for Treasury...");
    ...
    async function deployContract(wasmPath) {
        ...
        console.log(`\n🎉 Deployed Treasury Contract ID: ${contractId}`);
        return contractId;
    }
    
    try {
        let treasuryWasm = path.join(__dirname, "../target/wasm32v1-none/release/treasury.wasm");
        if (!fs.existsSync(treasuryWasm)) {
            treasuryWasm = path.join(__dirname, "../target/wasm32-unknown-unknown/release/treasury.wasm");
        }
        const treasuryId = await deployContract(treasuryWasm);

        console.log("\n--- TREASURY DEPLOYMENT SUCCESS ---");
        console.log(`NEXT_PUBLIC_SOROBAN_TREASURY="${treasuryId}"`);
        ...
        envContent = envContent.replace(/^NEXT_PUBLIC_SOROBAN_TREASURY=.*$/m, "");
        envContent += `\nNEXT_PUBLIC_SOROBAN_TREASURY="${treasuryId}"\n`;
        
        fs.writeFileSync(envPath, envContent.trim() + "\n");
        console.log("Updated apps/web/.env.local with NEXT_PUBLIC_SOROBAN_TREASURY");
