...
async function deploy() {
    console.log("Generating deployer keypair for Invoice Factoring...");
    ...
    async function deployContract(wasmPath) {
        ...
        console.log(`\n🎉 Deployed Invoice Factoring Contract ID: ${contractId}`);
        return contractId;
    }
    
    try {
        let factoringWasm = path.join(__dirname, "../target/wasm32v1-none/release/invoice_factoring.wasm");
        if (!fs.existsSync(factoringWasm)) {
            factoringWasm = path.join(__dirname, "../target/wasm32-unknown-unknown/release/invoice_factoring.wasm");
        }
        const factoringId = await deployContract(factoringWasm);

        console.log("\n--- FACTORING DEPLOYMENT SUCCESS ---");
        console.log(`NEXT_PUBLIC_SOROBAN_FACTORING="${factoringId}"`);
        ...
        envContent = envContent.replace(/^NEXT_PUBLIC_SOROBAN_FACTORING=.*$/m, "");
        envContent += `\nNEXT_PUBLIC_SOROBAN_FACTORING="${factoringId}"\n`;
        
        fs.writeFileSync(envPath, envContent.trim() + "\n");
        console.log("Updated apps/web/.env.local with NEXT_PUBLIC_SOROBAN_FACTORING");
