...
async function deploy() {
    console.log("Generating deployer keypair for Yield Sweeper...");
    ...
    async function deployContract(wasmPath) {
        ...
        console.log(`\n🎉 Deployed Yield Sweeper Contract ID: ${contractId}`);
        return contractId;
    }
    
    try {
        let sweeperWasm = path.join(__dirname, "../target/wasm32v1-none/release/yield_sweeper.wasm");
        if (!fs.existsSync(sweeperWasm)) {
            sweeperWasm = path.join(__dirname, "../target/wasm32-unknown-unknown/release/yield_sweeper.wasm");
        }
        const sweeperId = await deployContract(sweeperWasm);

        console.log("\n--- SWEEPER DEPLOYMENT SUCCESS ---");
        console.log(`NEXT_PUBLIC_SOROBAN_SWEEPER="${sweeperId}"`);
        ...
        envContent = envContent.replace(/^NEXT_PUBLIC_SOROBAN_SWEEPER=.*$/m, "");
        envContent += `\nNEXT_PUBLIC_SOROBAN_SWEEPER="${sweeperId}"\n`;
        
        fs.writeFileSync(envPath, envContent.trim() + "\n");
        console.log("Updated apps/web/.env.local with NEXT_PUBLIC_SOROBAN_SWEEPER");
