...
async function deploy() {
    console.log("Generating deployer keypair for Invoice Router...");
    ...
    async function deployContract(wasmPath) {
        ...
        console.log(`\n🎉 Deployed Invoice Router Contract ID: ${contractId}`);
        return contractId;
    }
    
    try {
        let invoiceWasm = path.join(__dirname, "../target/wasm32v1-none/release/invoice_router.wasm");
        if (!fs.existsSync(invoiceWasm)) {
            invoiceWasm = path.join(__dirname, "../target/wasm32-unknown-unknown/release/invoice_router.wasm");
        }
        const invoiceId = await deployContract(invoiceWasm);

        console.log("\n--- INVOICE ROUTER DEPLOYMENT SUCCESS ---");
        console.log(`NEXT_PUBLIC_SOROBAN_INVOICE="${invoiceId}"`);
        ...
        envContent = envContent.replace(/^NEXT_PUBLIC_SOROBAN_INVOICE=.*$/m, "");
        envContent += `\nNEXT_PUBLIC_SOROBAN_INVOICE="${invoiceId}"\n`;
        
        fs.writeFileSync(envPath, envContent.trim() + "\n");
        console.log("Updated apps/web/.env.local with NEXT_PUBLIC_SOROBAN_INVOICE");
